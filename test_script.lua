...
local triggerStartLightsButton
local falseStartButton
...
local initTimeoutId
local updateGrantedUsers
local toggleCompetitionModeEvent
local startLightsEvent
local requestLightsData
...
function script.initApp()
  if ac.isLuaAppRunning("Traffic_Lights") then
    ac.uninstallApp("Traffic_Lights")
  end
  triggerStartLightsButton = ac.ControlButton('Start_Lights_TRIGGER_START_LIGHTS',
  { keyboard = { key = (ui.KeyIndex.A) } })
  falseStartButton = ac.ControlButton('Start_Lights_TOGGLE_FALSE_START',
  { keyboard = { key = (ui.KeyIndex.F) } })
  toggleCompetitionModeEvent = ac.OnlineEvent({
  ...
  updateGrantedUsers = ac.OnlineEvent({
  ...
  startLightsEvent = ac.OnlineEvent({
  ...
  requestLightsData = ac.OnlineEvent({
    key = ac.StructItem.key("Start_Lights_request_data_events"),
  }, function(sender, data)
    if cannotRun() or sender.index == 0 then return end
    local adminCount = #admins
    local grantedUsersCount = #grantedUsers
    if table.contains(admins, sender.sessionID) then
      adminCount = adminCount - 1
    end
    if table.contains(grantedUsers, sender.sessionID) then
      grantedUsersCount = grantedUsersCount - 1
    end
    if ((adminCount > 0 or grantedUsersCount > 0) and not (isAdmin())) or not slMgr.trackHasLightMesh() then
      return
    end
